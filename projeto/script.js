...
async function loadDatabase() {
  try {
    ...
    // Mescla os projetos do db.json com os já salvos: garante que novos
    // projetos do db.json apareçam e mantém os publicados pelo usuário.
    const stored = JSON.parse(localStorage.getItem("portalProjetos") || "[]");
    const dbIds = new Set((db.projetos || []).map((p) => p.id));
    const publicadosPeloUsuario = stored.filter((p) => !dbIds.has(p.id));
    const merged = [...(db.projetos || []), ...publicadosPeloUsuario];
    localStorage.setItem("portalProjetos", JSON.stringify(merged));
  } catch (error) {
...
function liberarAcessoPortal() {
  sessionStorage.setItem("regiimento_acesso", "1");
}

function setupLogin() {
  const form = document.getElementById("loginForm");
  const guest = document.getElementById("guestBtn");
  if (!form && !guest) return; // não é a página de login

  // "Visualizar sem criar conta" e qualquer link para a home liberam o acesso
  document.querySelectorAll('a[href="../index.html"]').forEach((a) => {
    a.addEventListener("click", liberarAcessoPortal);
  });
...
function getForumPosts() {
  ...
  // Mescla com tópicos criados pelo usuário (mantém os novos do db.json)
  const stored = JSON.parse(localStorage.getItem("portalForum") || "[]");
  const seedIds = new Set(seed.map((s) => s.id));
  const criadosPeloUsuario = stored.filter((p) => !seedIds.has(p.id));
  return [...seed, ...criadosPeloUsuario];
}
